...
export default class IdsDataGridFilters {
   ...
   * @returns {string} The resuling template
   */
  text(column) {
   ...
   * @returns {HTMLElement|null} The filter wrapper element
   */
   ...
   * @param {Array<object>} conditions An array of objects with the filter conditions.
  ...
  applyFilter(conditions) {
    ...
    if (!conditions.length && !this.root.datasource.filtered) {
    ...
    const checkRow = (row, index) => {
      ...
      for (let i = 0, l = conditions.length; i < l; i++) {
        ...
        const stringVal = (v) => ((v === null || v === undefined) ? '' : v.toString().toLowerCase());
        const formatterVal = () => {
  ...
  attachFilterEventHandlers() {
    ...
    this.root.onEvent(`selected.${this.#id()}`, this.root.elements?.header, (e) => {
    ...
    this.root.onEvent(`change.${this.#id()}`, this.root.elements?.header, (e) => {
  ...
  #operators(type, column) {
    ...
    const contentsOperators = () => {
      const val = this.root.datasource.allData.map((row) => row[column.id]);
