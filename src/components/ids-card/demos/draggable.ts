...
const rightPanel = document.querySelector('#example-panel-right')!;
const centerPanel = document.querySelector('#example-panel-center')!;
...
[...containers, ...components].forEach((item) => {
  const componentEl = createComponent(item, '140px');
  centerPanel.appendChild(componentEl);
});


[...containers, ...components].forEach((item) => {
  const componentEl = createComponent(item);
  rightPanel.appendChild(componentEl);
});
