...
class IdsMonthView extends Base {
  ...
  #attachEventHandlers() {
    ...
    this.onEvent('click.month-view-dayselect', this.container.querySelector('tbody'), (e: any) => {
  ...
  #attachToolbarEvents() {
    ...
    this.onEvent('click.month-view-buttons', buttonSet, (e: any) => {
      e.stopPropagation();
    ...
    this.onEvent('dayselected.month-view-datepicker', this.container.querySelector('ids-date-picker'), (e: any) => {
   ...
   * @param {string} type of event to be called of 'next'|'previous'|'today'
   */
   ...
   */
  #daySelectClick(element: HTMLElement) {
    if (!element) return;
    ...
    if (!(isSelected || isDisabled)) {
      ...
      if (this.#isRange()) {
        this.#selectDay((year as any), (month as any), (day as any));
      } else {
        this.day = (day as any);
      }

      // Alternate cells clicked
      if ((stringToNumber((month as any)) !== this.month || this.locale?.isIslamic()) && !this.#isRange()) {
        this.year = (year as any);
        this.month = (month as any);
      }
   ...
   */
  #monthInDayFormat(date: Date, rangeStartsOn: Date,) {
   ...
   */
  #getCellTemplate(weekIndex: number) {
    const firstDayOfRange: any = this.#isRange()
      ? this.startDate
    ...
    return Array.from({ length: WEEK_LENGTH }).map((_, index) => {
      ...
      const isDisabled = this.#isRange() && (date < (this.startDate as any) || date > (this.endDate as any));
      const isAlternate = !this.#isRange() && (date < firstDayOfRange || date > (lastDayOfRange as any));
  ...
  #renderWeekDays() {
    ...
    const weekDaysTemplate = weekDays.map((elem: any, index: number) => {
  ...
  set firstDayOfWeek(val) {
    const numberVal = stringToNumber((val as any));
