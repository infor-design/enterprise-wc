...
export default class IdsAutoComplete extends Base {
  ...
  displayMatches() {
    ...
    const results = resultsArr.map((result) => {
      ...
      return `<ids-list-box-option value="${result[this.searchKey]}">${optionText}</ids-list-box-option>`;
    }).join('');
  ...
  #attachEventListeners() {
    this.onEvent('blur', this.input, (e) => {
      e.preventDefault();
    });
    this.onEvent('click', this.listBox, this.handleSelectOption.bind(this));
