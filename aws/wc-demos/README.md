...
```bash
sam local invoke WCDemosLambdaFunction --no-event
```
...
Resources:
  MyQueue:
    Type: AWS::SQS::Queue
  WCDemosLambdaFunction:
    Type: AWS::Serverless::Function
    Properties:
      Handler: src/handlers/index.LambdaHandler
      Runtime: nodejs14.x
      DeadLetterQueue:
        Type: SQS
...
```bash
sam logs -n WCDemosLambdaFunction --stack-name sam-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name wc-demos
```
