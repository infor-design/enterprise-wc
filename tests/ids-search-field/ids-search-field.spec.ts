...
test.describe('IdsSearchField tests', () => {
  ...
  test.describe('general page checks', () => {
    test('should have a title', async ({ page }) => {
      await expect(page).toHaveTitle('IDS SearchField Component');
    });
  ...
  test.describe('snapshot tests', () => {
    test('should match innerHTML snapshot', async ({ page, browserName }) => {
      ...
      const handle = await page.$('ids-search-field');
      const html = await handle?.evaluate((el: IdsSearchField) => el?.outerHTML);
    ...
    test('should match shadowRoot snapshot', async ({ page, browserName }) => {
      if (browserName !== 'chromium') return;
      const handle = await page.$('ids-search-field');
      const html = await handle?.evaluate((el: IdsSearchField) => {
