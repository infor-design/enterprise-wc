...
describe('Ids Blockgrid Percy Tests', () => {
  const url = 'http://localhost:4444/ids-block-grid/example.html';

  it('should not have visual regressions in new light theme (percy)', async () => {
    await page.goto(url, { waitUntil: ['networkidle2', 'load'] });
    await pageSnapshot(page, 'ids-block-grid-new-light');
  });
  ...
  it.skip('should not have visual regressions in new dark theme (percy)', async () => {
    ...
    await pageSnapshot(page, 'ids-block-grid-new-dark');
  });
  ...
  it.skip('should not have visual regressions in new contrast theme (percy)', async () => {
    ...
    await pageSnapshot(page, 'ids-block-grid-new-contrast');
  });
});
