describe('Ids Date Picker e2e Tests', () => {
  ...
  it('should handle calendar popup events', async () => {
    ...
    // Click outside
    await page.evaluate(() => {
      (document.querySelector('ids-container') as any).click();
    });
  ...
  it('calendar popup should set correct input date value', async () => {
    ...
    await page.evaluate(() => {
      ...
      const monthView = (component?.shadowRoot as any).querySelector('ids-month-view');
      const day = (monthView?.shadowRoot as any).querySelector('td[data-day="31"]');
      day?.click();
    });
    ...
    await page.evaluate(() => {
      ...
      (component?.shadowRoot as any).querySelector('.popup-btn-end')?.click();
    });
  ...
  it('should change when used in calendar toolbar', async () => {
    ...
    await page.evaluate(() => {
      (document.querySelector('ids-container') as any).insertAdjacentHTML('afterbegin', `
        <ids-date-picker
    ...
    await page.evaluate(() => {
      const component: any = document.querySelector('#e2e-datepicker-toolbar');
  ...
  it('should handle validation', async () => {
    ...
    await page.evaluate(() => {
      (document.querySelector('#e2e-datepicker-required') as any).validate = null;
      (document.querySelector('#e2e-datepicker-required') as any).validationEvents = null;
    });
    ...
    await page.evaluate(() => {
      (document.querySelector('#e2e-datepicker-required') as any).validationEvents = 'blur';
    });
  ...
  it('should handle locale change', async () => {
    ...
    await page.evaluate(() => {
      (document.querySelector('ids-container') as any)?.setLocale('ar-SA');
    });

    // Wait till calendars load
    await page.waitForFunction(() => ((document.querySelector('ids-container') as any)?.locale as any)?.calendar().name === 'islamic-umalqura');
    ...
    await page.evaluate(() => {
      (document.querySelector('ids-container') as any).setLocale('ar-EG');
    });

    // Wait till calendars load
    await page.waitForFunction(() => (document.querySelector('ids-container') as any)?.locale?.calendar().name === 'gregorian');
    ...
    await page.evaluate(() => {
      (document.querySelector('ids-container') as any)?.setLocale('en-US');
    });
