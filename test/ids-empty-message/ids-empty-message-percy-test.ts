...
describe('Ids Empty Message Percy Tests', () => {
  const url = 'http://localhost:4444/ids-empty-message/example.html';

  it('should not have visual regressions in new light theme (percy)', async () => {
    await page.goto(url, { waitUntil: ['networkidle2', 'load'] });
    await pageSnapshot(page, 'ids-empty-message-new-light');
  });
  ...
  it('should not have visual regressions in new dark theme (percy)', async () => {
    ...
    await pageSnapshot(page, 'ids-empty-message-new-dark');
  });
  ...
  it('should not have visual regressions in new contrast theme (percy)', async () => {
    ...
    await pageSnapshot(page, 'ids-empty-message-new-contrast');
  });
});
