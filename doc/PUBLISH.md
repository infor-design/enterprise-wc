...
- Search for `1.0.0-beta.n` (current version) and replace with new version (currently `1.0.0-beta.18.3`)
- It will be in`package.json`, `package-dist.json`, about tests, and `src/core/ids-attributes.js` and this file.
- Commit and push
- Make and push a tag with `git tag 1.0.0-beta.18.3 && git push origin --tags`
...
- Run command `gh release create 1.0.0-beta.18.3 --title "1.0.0-beta.18.3" --notes-file "doc/CHANGELOG.md"`
